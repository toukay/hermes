...
async def get_sub_durations() -> list[SubDuration]:
    async with get_session() as session:
        return await session.execute(select(SubDuration)).scalars().all()
    
async def get_sub_duration(duration: int, unit: str) -> SubDuration:
    async with get_session() as session:
        return await session.execute(select(SubDuration).filter(SubDuration.duration == duration, SubDuration.unit == unit)).scalar_one_or_none()
...
async def create_subscription(user: User, duration: SubDuration) -> Subscription:
    async with get_session() as session:
        unit = 1 if duration.unit == 'day' else 30 if duration.unit == 'month' else 0
        subscription = Subscription(user.id, datetime.now(), datetime.now() + timedelta(days=duration.duration * unit))
        session.add(subscription)
    return subscription
...
async def get_unique_code_by_code(code: str) -> UniqueCode:
    async with get_session() as session:
        return await session.execute(select(UniqueCode).filter(UniqueCode.code == code)).scalar_one_or_none()
...
async def delete_expired_unique_codes() -> None:
    async with get_session() as session:
        await session.execute(select(UniqueCode).filter(UniqueCode.redeemed == False, UniqueCode.expiration_date < datetime.now())).delete()
